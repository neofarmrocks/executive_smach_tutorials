...
def construct_sm(node: Node):
    ...
    with sm:
        ...
        StateMachine.add(
            "TUTORIAL_IT", tutorial_it, {"succeeded": "succeeded", "aborted": "aborted"}
        )
    ## %EndTag(ADDITERATOR)%
    return sm
...
def main(args=None):
    ...
    node = rclpy.create_node("iterator_tutorial")
    sm_iterator = construct_sm(node)

    # Run state machine introspection server for smach viewer
    intro_server = IntrospectionServer(
        "iterator_tutorial", sm_iterator, "/ITERATOR_TUTORIAL"
    )
    intro_server.start()
